...
def parse_multipart_form(handler):
    ...
    for part in body.split(delimiter):
        ...
        headers = {}
        for raw_line in raw_headers.decode("utf-8", errors="ignore").split("\r\n"):
            if ":" not in raw_line:
                continue
            key, header_value = raw_line.split(":", 1)
    ...
    def do_POST(self):
        ...
        if self.path == "/api/extract":
            try:
                ...
                api_key = fields.get("api_key", "").strip() or os.environ.get("OPENAI_API_KEY", "").strip()
                if not api_key:
                    raise ValueError("OPENAI_API_KEY is missing. Paste it into the form or set it in your environment.")

                base_url = fields.get("base_url", "").strip() or "https://api.openai.com/v1"
                model = fields.get("model", "").strip() or "gpt-4.1-mini"
                group_a_name = fields.get("group_a_name", "").strip() or "Treatment A"
                group_b_name = fields.get("group_b_name", "").strip() or "Treatment B"
